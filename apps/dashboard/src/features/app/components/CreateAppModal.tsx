...
type Props = {
  isOpen: boolean
  onClose: () => void
  onNewApp: (app: App) => void
}

export const CreateAppModal: React.FC<Props> = ({
  isOpen,
  onClose,
  onNewApp,
}) => {
  const scopedT = useTranslations('app.createModal')
  const { showToast } = useToast()
  const { organization } = useOrganization() 
  const { mutate, isLoading } = trpc.app.create.useMutation({
